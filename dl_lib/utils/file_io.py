...
class OSSPathHandler(PathHandler):
    """
    Download OSS File and cache them to dist.
    """
    # def __init__(self) -> None:
    #    self.cache_map: Dict[str, str] = {}
    ...
    def _open(self, path: str, mode: str = "r") -> IO[Any]:
        ...
        assert self._isfile(path), r"{path} is not an oss file"
        oss_path = oss.OSSPath(path)
        return oss_path.download()

    def _exists(self, path: str) -> bool:
        return oss.OSSPath(path).exists()

    def _isfile(self, path: str) -> bool:
        return oss.OSSPath(path).is_file()

    def _isdir(self, path: str) -> bool:
        return oss.OSSPath(path).is_dir()

    def _ls(self, path: str) -> List[str]:
        return list(oss.OSSPath(path).list_all())
...
PathManager.register_handler(OSSPathHandler())
